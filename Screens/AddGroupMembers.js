...
const AddGroupMembers = () => {
  const { chat, selectedConversation } = useContext(Context);
  ...
  const searchGroupMembers = (userList) => {
    ...
    const limit = 30;
    const groupMemberRequest = new chat.GroupMembersRequestBuilder(GUID)
      .setLimit(limit)
      .build();
  ...
  const searchUsers = () => {
    if (chat) {
      const limit = 30;
      const usersRequestBuilder = new chat.UsersRequestBuilder().setLimit(limit);
  ...
  const handleAddMember = (selectedUser) => {
    if (shouldAddMember(selectedUser)) {
      ...
      const GUID = selectedConversation.guid;
      const membersList = [
        new chat.GroupMember(selectedUser.uid, chat.GROUP_MEMBER_SCOPE.PARTICIPANT),
      ];
      chat.addMembersToGroup(GUID, membersList, []).then(
        response => {
          setIsLoading(false);
