...
const CreateGroup = () => {
  ...
  const { setUser, chat } = useContext(Context);
  ...
  const createGroup = () => {
    if (isGroupValid(groupName)) {
      setIsLoading(true);
      const GUID = uuidv4();
      const groupType = chat.GROUP_TYPE.PUBLIC;
      const groupIcon = generateAvatar();
      const password = "";

      const group = new chat.Group(GUID, groupName, groupType, password);
      group.setIcon(groupIcon);

      chat.createGroup(group).then(
        group => {
          setIsLoading(false);
          showMessage('Thông báo:', `${groupName} đã được tạo thành công`);
        },
        error => {
          setIsLoading(false);
          showMessage('Lỗi', 'Không thể tạo nhóm,hãy thử lại sau!');
        }
      );
    }
  };
